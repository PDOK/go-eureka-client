...
func NewInstanceInfo(instanceId string, hostName string, app string, ip string, port int, isSsl bool, status string) *InstanceInfo {
...
func (c *Client) SendRequest(rr *RawRequest) (*RawResponse, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		logger.Debugf("recv.response.from %s with status code %d ", httpPath, resp.StatusCode)
